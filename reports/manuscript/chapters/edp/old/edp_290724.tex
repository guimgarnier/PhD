...
Mutations have played an important role in the evolution of the different species. In~\cite{robert2018mutation}, the authors have considered a microfluidic setting to study the impact of a new mutation on the fitness of a cell. \ndr{ref}


\medskip
In the mathematical litterature, many authors have considered models to. 
\ndr{Bertoin, Raoul}

\medskip

In this article, we study a model derived from the experimental model proposed by Lydia Robert. \\ 

In this model, we consider a population of cell $u(t,x)$ evolving through the time $t \in \RR_+$ and structured by the growth rate $x \in \RR_+$.  
Mutations are assumed to happen at a constant rate $\mu$. This assumption comes from biological evidences \ndr{ref} \\

In this article, we assume that ...
$k(x,y) := k(\frac{x}{y})$. This assumption is referred as the \textit{'self-similarity property'}. 

We aim to understand the asymptotic behavior of $u$ when $t \to \infty$. En particulier, nous voudrions comprendre. \\
Dans le cas où la fitness s'échappe vers 0 ou l'infini, nous voudrions pouvoir estimer une vitesse d'échappemement. 

The motivation for this study is the fact that we want to understand what is the good model to explain the stability that we observe in the real life. 

\subsection{Review on existing results}
...
When all mutations are deleterious, the support of $k$ is contained is $(0,1)$. In this case, Equation~\ref{} is kwown has the fragmentation equation for wich many studies have already be done. 

\subsection{Main results and organization of the article}
...
\begin{itemize}
    \item An extension of the work done \ndr{ref} when
    \item 
\end{itemize}
...
For this, our aim is to find for any $z \in \CC$ such that $\Re(z) > 0$ a function $L(\cdot,z)$ analytic in the strip $S = \{\xi \in \CC \mid \Re(\xi) \in (s_0, s_0+1)\}$. 
