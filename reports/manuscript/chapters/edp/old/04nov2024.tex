...
\begin{itemize}
    ...
    \item Reformulation de l'étape 2 de Wiener Hopf après la section \ref{s:reformulate_eq} 
    \item Reformulation du théorème \ref{th:SolutionWH}. 
    \item Quelques explications supplémentaire sur la raison pour laquel l'Equation \ref{def:H} est bien définie. 
    \item Plus d'explication biologique sur le sens de l'hypothèse \ref{hyp:boundK}
    \item Ajout d'une partie Notation dans l'introduction. En particulier on définit les principaux résultats de la transformées de Mellin. 
    \item Refonte complête de la section \ref{s:lineage}
    \item Ajout d'un appendix pour les preuves de la section \ref{s:lineage} qui sont des adaptations des résultats dans \cite{doumic2016time}.
\end{itemize}
...
Another main difference between our work and existing work is that we consider that mutations have a multiplicative effect on fitness, whereas most authors focus on additive effects. This multiplicative assumption comes from the model proposed in \cite{robert2018mutation}. This assumption means that we do not have a convolution term but instead have a multiplicative convolution term. As far as we know, there is no study that focus on multiplicative effects. To study this ‘multiplicative’ term, we use the Mellin transformation. 
...
\begin{equation}
    ...
    \Mcal[f](s) = \int_{\RR_+} x^{s-1} f(x) \diff x \eqfinv
\end{equation}
...
\begin{lemma}\label{lm:interval_mellin}
    ...
    $$ 
    is an open interval of $\RR$ and the application $s \mapsto \Mcal_f(s)$ is analytic on the strip $I(f) \times i\RR$. 
...
\label{s:lineage}
    ...
    \end{align}
    and $k_0:   \RR_+ \to \RR$ is designed as the kernel of mutation. We suppose that 
    \begin{align}
        \label{H:k_0}
        k_0 \geq 0 \eqsepv \int_0^\infty k_0(x)(1+x)\diff x < \infty \mtext{and} \int_0^\infty k_0(x)\diff x = 1 \eqfinp
    \end{align}

    The main tool to study Equation~\eqref{eq:main} is the Mellin transform that we defined in \eqref{e:mellin_def_function}. Indeed, by applying the Mellin transform to Equation~\eqref{eq:main}, we have
	\begin{equation*}
        \begin{cases}
    ...
    \begin{equation*}
        \begin{split}
        U(t,s) \coloneqq \int_0^\infty x^{s-1} u(x,t) \diff x 
    ...
    \begin{equation*}
        ...
        \eqfinp
    \end{equation*}
    We note that $K$ is a convex function, such that $K(1) = 1$.
   ...
   \subsubsection{A heuristic explanation of the deterministic model} In \cite{robert2018mutation}, the authors observe several trajectories of a multiplicative compound Poisson process 
   \begin{equation*}
   W_t =W_0 \prod_{i = 1}^{N_t}(1 - s_i) 
   \eqfinv
   \end{equation*}
   where $(N_t)_{t \geq 0}$ is a Poisson process with intensity $\mu$ independent of the $\iid$ random variables $(1-s_i)$ with density $k$. \\
   Let $u(t,x)$ be the probability distribution of $W_t$. Beetween a time $t$ and $t + dt$, the jump probability is $\mu dt$. So
   \begin{align*}
       u(t + dt,x) 
       &= (1  - \mu dt) u(t,x) + \mu dt \int_0^\infty k(y,x) u (t, y) \dy
   \end{align*}
   ...
   \begin{equation*}
       ...
       =   - \mu u(t,x) + \mu \int_0^\infty k(y,x) u (t, y) \dy  
       \eqfinp
   \end{equation*}
   The assumption that $k$ is self-similar leads to $k(y,x) = \frac{1}{y}k_0\big(\frac{x}{y}\big)$, then 
   We could also have written that 
   \begin{align*}
       u(t + dt,x) 
       &= (1  - \mu dt) u(t,x) + \mu dt \int_0^\infty \frac{1}{y}k_0\big(\frac{x}{y}\big) u (t, y) \dy
       \eqfinp
   \end{align*}
%    \begin{equation*}
%          u(t + dt,x)  = (1  - \mu dt) u(t,x) + \mu dt \int_0^\infty \frac{1}{y}k_0(y) u \Big( t, \frac{x}{y}\Big) \dy
%    \end{equation*}
%    We perform the change of variable $z = x/y$, $\dz = -x/y^2 \dy$. Then 
%    \begin{align*}
%        u(t + dt,x)  
%        &= (1  - \mu dt) u(t,x) + \mu dt \int_0^\infty \frac{z}{x}k_0\Big(\frac{x}{z}\Big) u ( t, z) \frac{1}{x} \frac{x^2}{z^2 }\dz\\
%        &= (1  - \mu dt) u(t,x) + \mu dt \int_0^\infty \frac{1}{z}k_0\Big(\frac{x}{z}\Big) u ( t, z) \dz        
%    \end{align*}
%    By dividing  by $dt$ ($dt \to 0$), it follows that 
%    \begin{equation*}
%        \frac{\partial}{\partial t} u(t,x) 
%        = 
%        -\mu u(t,x)
%        + 
%        \mu \int_0^\infty \frac{1}{z}k_0\Big(\frac{x}{z}\Big) u ( t, z) \dz     
%    \end{equation*}
   This is the Cauchy problem \eqref{eq:main}.    


    \subsubsection{The choice of the kernel}

    The choice of the mutation kernel $k$ in Equation~\eqref{eq:main} has an impact on the solution. Indeed, if the support of $k$ is included in $[0, 1]$, then no beneficial mutations can happen in the population. The fitness of a cell can only decrease upon the time. 

    In \cite{robert2018mutation}, the authors approximate the expression of $k$ from experimental observations. However, they assume that all mutations are deleterious. They were able to calculate the first moments of the law of $k$, and found that it could be close to a beta law. \\

    In the rest of this section, we give different examples of kernel, and for each of them we explain what its biological interpretation would be. We also calculate the Mellin transform of each of these kernels. The reasons for these calculations will become clearer later in the article. \\
...
\end{align*}
    ...
    %However, there is no guarantee that $K$ admits a local minimum \cite{doumic2016time}. The following lemma shows that if $K$ is strictly monotonic then all mutations are beneficial or all mutations are deleterious. Since we want to study kernels that can have both beneficial and deleterious effects, we assume that $K$ has a unique minimum reach in $s_* \in \RR$.
        ...
        \begin{proposition}
            \label{p:locationmoment} Let $n\geq 1$ be an integer. Suppose that $K$ is minimum in $s_* \in (0, \infty)$, then for all $R > 0$
            \begin{enumerate}
                \item If $n$ = 1. 
                \begin{enumerate}
        ...
        \begin{proof}
            ...
            \textit{(a) Suppose that $s_* < 1$.} Let $c$ be a real $>1$. 
            ...
            \textit{(b) Suppose that $s_* > 1$}. Let $c$ be a real $<1$. \\
            By Mellin inversion, we have that
            \begin{align*}
                u(t,x) = \frac{1}{2i\pi}\int_{c-i\infty}^{c+i\infty} U_0(s) x^{-s} e^{-\mu t(1 - K(s))} \diff s
            \end{align*}
        ...
        \textit{(a) Suppose that $s_* > n$}. Let $c$ be a real $<n$. \\
            By Mellin inversion, we have that
            \begin{align*}
                u(t,x) = \frac{1}{2i\pi}\int_{c-i\infty}^{c+i\infty} U_0(s) x^{-s} e^{-\mu t(1 - K(s))} \diff s
            \end{align*}
        ...
        \textit{(b) Suppose that $s_* < n$.} Let $c$ be a real $>n$. \\
            By Mellin inversion, we have that
            \begin{align*}
                u(t,x) = \frac{1}{2i\pi}\int_{c-i\infty}^{c+i\infty} U_0(s) x^{s} e^{-\mu t(1 - K(s))} \diff s
            \end{align*}
        ...
        Considering the case $n = 1$, Proposition~\ref{p:locationmoment} allows us determine the fitness distribution of the population. Indeed, when $s_* > 1$, the proposition ensures that for all $R > 0$, 
        \begin{align*}
            \lim_{t \to \infty} \int_R^\infty u(t,x) \diff x = \int_0^\infty u_0(x) \diff x \eqfinp
        \end{align*} 
        This ensures that for $t \in \RR_+$ enough large, the set $\{ x \in (R, \infty) \,;\, u(x,t) > 0\}$ has measure $0$. As this is true for all real $R > 0$, it means that after a sufficiently long time, the number of cells with a positive fitness is negligible, \ie the fitness distribution of all the cells converges towards a Dirac mass in 0. \\

        Similarly, if $s<s_*$, the proposition ensures that 
        \begin{align*}
            \lim_{t \to \infty} \int_0^R u(t,x) \diff x = \int_0^\infty u_0(x) \diff x \eqfinp
        \end{align*} 
        This ensures that for any compact set $ C \subset (0, \infty)$, the set  $$\{ x \in C \,;\, u(x,t) > 0\}$$ has measure $0$ when $t \in \R_+$ is enough large. For any real $A > 0$, this means that the whole population has a fitness greater than $A$ after a sufficiently long time.

        \begin{example}
            Consider the uniform mutation kernel $k_0(x)$
            \ndr{développer}
        \end{example}

    \subsection{An explicit solution of the Cauchy problem}
    \label{s:explicit}
    ...
    \eqfinp
    \end{equation}
We denote the fundamental strip of $u_0$. 
\begin{equation}
...
\begin{theorem}(Existence of an explicit solution) 
    ...
    \begin{equation}
        ...
        u(t,x) = \frac{1}{2\pi i} \int_{\nu-i\infty}^{\nu+i\infty} U_0(s) e^{(K(s)-1)t}x^{-s} \diff s 
    ...
    \subsection{The asymptotic behaviour of $u(t,x)$}\mbox{} 

    In this section, we want to characterize the asymptotic behaviour of $u$ when $x \to \infty$ and $t \to \infty$. A similar study has been done in \cite{doumic2016time} for the fragmentation equation. We extend this study to the mutation equation where the support of $k$ is not necessarily included in $[0, 1]$. 
    The key point to study $u$ is the steepest descent method which has been developed by Peter Debye in 1909 \cite{debye1909naherungsformeln} to study Bessel functions. We give a quick description of the method in Section~\ref{s:mth_col}. A full description of the method can be found in \cite{dieudonne1980calcul}. 
        ...
        \begin{align*}
            w(t,x) \coloneqq e^t u(t,x) 
            \mtext{for} t \in \RR_+ \eqsepv x \in \RR_+
            \eqfinv
        \end{align*} which is easier to obtain. 
...
Our aim is to study the asymptotic behaviour of $w$ as $t$ becomes larger and larger. For that, we apply the method of steepest descent which leads us to consider the curve $(t,x) \mapsto s_+(x,t)$ where $\partial_s \phi(s_+, \cdot,\cdot)= 0$. 
...
\begin{theorem}\label{t:asymp_lineage}
    Suppose that  $u_0$ satisfies \eqref{H:u0_L1}, \eqref{H:DoumicEsco22} and \eqref{H:DoumicEsco23}. 
...
In order to understand the behaviour of the function $u(t, x)$ as $t$ increases we may consider the curves $C_r$ in the plane $(x, t)$ defined by $s_+(t, x) = r$ for any constant $r \in \RR$. As \begin{align*}
    s_+(t,x) \coloneqq (K')^{-1} \Big( \frac{\log x}{\mu t} \Big)
    \mtext{for} t \in \RR_+ \eqsepv x \in \RR_+
    \eqfinv
\end{align*}
the curve $C_r$ correspond to the set of the points $(x, t)$ where $x = e^{\mu K'(r) t}.$

Let denotes $A(t,s_+) = \frac{U_0(s_+(t,x)+\omega_\delta(\varepsilon))}{\sqrt{2\pi tK''(s_+)}}$. 
We observe that 
    \begin{align*}
        u(t,x)
        &\approx
        A(t,s_+) e^{(K(s_+)-1)t -  \mu s_+ t K'(s_+)} \\
        &\approx A(t,s_+) e^{[K(s_+)-1 -  \mu s_+ K'(s_+)]t}
        \eqfinp
    \end{align*}
    The sign of 
    \begin{equation}\label{eq:DET91}
        F(s_+) \coloneqq[K(s_+)-1 -  \mu s_+ K'(s_+)]
    \end{equation} implies the exponential growth or decay of $u(t, x)$ on the curve $\Ccal_{s_+}$. \\

    \ndr{We observe that $\lim\limits_{s\to -\infty}K(s) = \infty$ and $\lim\limits_{s\to-\infty} K'(s) = -\infty$. However, we can't compute $\lim\limits_{s\to -\infty}F(s)$ because we have $\lim\limits_{s\to -\infty}F(s) = \infty - \infty$. For that, we need to compare $$K(s) = \int_0^\infty k_0(z)z^{s-1} \diff z$$ and $$sK'(s) = s \int_0^\infty \ln(z)k_0(z)z^{s-1} \diff z$$ Simulations seems to show that $F(s) \to -\infty$}

    % \begin{lemma}
    %     The function $F$ defined by \eqref{eq:DET91} has at least one zero in $(-\infty, s_*)$. Moreover if $s_* > 1$, then $F$ has at least one zero in $(s_*, \infty)$. 
        
    % \end{lemma}
    % \begin{proof}
    %     We first notice that for all $s < s_*$, $K'(s) < 0$ and $\lim\limits_{s \to -\infty} K(s) = \infty$, which ensures that 
    %     \begin{equation*}
    %         \lim\limits_{s \to -\infty} F(s) = \infty
    %         \eqfinp
    %     \end{equation*} 
    %     We also have that $F(s_*) = K(s_*)-1 < 0$ which ensures by continuity that $F$ has at least one zero in $(-\infty, s_*)$. \\
    %     Moreover, suppose that $s_* > 1$. We have that $K'(1) < 0$. therefore
    %     \begin{align*}
    %         F(1) = K(1)-1 -  \mu K'(1) = - \mu K'(1) > 0
    %         \eqfinp
    %     \end{align*}
    %     By continuity, there is at least one zero in $(s_*, 1)$. 
    % \end{proof}
    ...
    We want to know where $F$ is maximal. For that, we compute 
    \begin{align*}
        F'(s) = K'(s) (1-\mu) - \mu s K''(s) = 0
    \end{align*}
    It follows that $ K'(s) = \frac{\mu sK''(s)}{1-\mu}$. 

    \ndr{The following examples illustrate that $F$ may have no zero or several of them. On a l'impression que $F$ tends toujours vers $-\infty$ quand $s \to \pm \infty$}
    ...
    \label{fig:sum_eps}
    \caption{\textbf{Left:}
    Function $F(s)$ for $k_0 =  \frac12 \delta_{1-\varepsilon} + \frac12 \delta_{1 + \varepsilon}$. We see there are two zeros. \textbf{Right:} Several curves of the form $s_+ = \gamma$ for different values of $\gamma$. Between the black curves, $u(t,x)$ tends to infinity. Outside, $u(t,x)$ tends to zero exponentially fast}
    \end{figure}

    \textbf{Example 2: The uniform kernel $k_0 =  \1{[0.1; 1.1]}$} \\
    For  the uniform kernel $k_0 =  \1{[0.1; 1.1]}$, we have $K(s) = \frac{1}{s}\cdot \frac{5^s-0.1^s}{5-0.1}$. Let $\mu = 0.7$. 
    ...
    \caption{\textbf{Left:}
    Function $F(s)$ for $k_0 =   \1{[0.1; 5]}$. We see there are two zeros. \textbf{Right:} Several curves of the form $s_+ = \gamma$ for different values of $\gamma$.  Between the black curves, $u(t,x)$ tends to infinity. Outside, $u(t,x)$ tends to zero exponentially fast }
    \end{figure}

    \textbf{Example 3: The uniform kernel $k_0 =  \1{[0.1; 5]}$} \\
    For  the uniform kernel $k_0 =  \1{[0.1; 5]}$, we have $K(s) = \frac{1}{s}\cdot \frac{5^s-0.1^s}{5-0.1}$. Let $\mu = 0.7$. 
    ...
    \caption{\textbf{Left:}
    Function $F(s)$ for $k_0 =   \1{[0.1; 5]}$. We see there is no zero. \textbf{Right:} Differents curves of the form $s_+ = \gamma$ for different values of $\gamma$. }
    \end{figure}



    % Similarly, for all $s > s_*$, $K'(s) > 0$ and $\lim\limits_{s \to \infty} K(s) = \infty$. It ensures that 
    % \begin{equation*}
    %     \lim\limits_{s \to \infty} F(s) = \infty
    %     \eqfinp
    % \end{equation*} 
    ...
    %     F(s) 
    %     &=  K(s) - 1 - \mu s K'(s)     \\
    %     &= \int_0^\infty k_0(z)z^{s-1} \diff z - 1 -  \mu s \int_0^\infty \ln(z) k_0(z)z^{s-1} \diff z \\
...
% \textbf{The sum of dirac mass} Consider the mutation kernel $$k_0(x) = 0.5\delta_\alpha + 0.5\delta_\beta$$ with $\beta > 1$ and $\alpha < 1.$
% Then for all $s \neq 0$, 
% \begin{align*}
%     K(s) = 0.5\alpha^{s-1}+ 0.5\beta^{s-1}
%     \eqfinp
% \end{align*}
% It follows that 
% \begin{align*}
%     K'(s) = 0.5\ln{\alpha} \cdot \alpha^{s-1} + 0.5\ln{\beta} \cdot \beta^{s-1}
%     \eqfinp
% \end{align*}
% \ndr{Faire des calculs... compliqués}

% \textbf{The uniform kernel} Consider the mutation kernel $k_0(x) = \frac{1}{b-a}\1{(a,b)}$, $a,b > 0$ \\
% Then for all $s \neq 0$, 
%     \begin{align*}
%         K(s) = \frac{1}{b-a}\int_a^b x^{s-1}\diff x = \frac1s \cdot\frac{b^s - a^s}{b-a}
%         \eqfinp
...
% It follows that 
% \begin{align*}
%     K'(s) =  \frac{-a^s + b^s + a^s s \ln a - b^s s \ln(b)}{(a - b) s^2}
%     \eqfinp
% \end{align*}
% \ndr{Faire des calculs... compliqués}
...
The aim is to find for any $z \in \CC$ such that $\Re(z) > 0$ a function $L(z, \cdot)$ analytic in the strip $S = \{\xi \in \CC \mid \Re(\xi) \in (s_0, s_0+1)\}$. The analyticity of $L(z, \cdot)$ strongly depends on the analytic properties of $K$. We present briefly the different steps of the Wiener--Hopf method. 
...
\begin{itemize}
    ...
    $ \Xi(z,x+i0) = \Xi(z,x-i0) $
    where $g$ is contained in the expression of $\Xi$. The goal is to obtain a expression that only depends on $x+i0$ (resp. $x-i0$) on the left (resp. right) side of the equality. This allows us to extend $\Xi$ on $\CC^*$ as a analytic function. 
     To obtain this expression, the key point is the Sokhotski--Plemelj formula \cite{sokhotski1873definite} that split a continuous function $f$ in two part $$f(x) = f_-(x-i0) + f_+(x+i0) \mtext{for} x \in (0,\infty)\eqfinp$$
    In the proof of Theorem~\ref{th:SolutionWH}, we obtain the expression $$\Xi(\cdot) = M(z,\cdot)g(z, \cdot) -W(z,\cdot)$$  where $M$ and $W$ will be defined later. 
...
The first step is to transform the Equation~\eqref{eq:wienerhopfeq} on a strip $[s_0, s_0+1] + i \RR$ into an equation on $\CC \setminus \RR_+$. For this, we introduce the variables:
    \begin{align}
...
\begin{hypothesis}\label{hyp:boundK} \mbox{}
    \begin{enumerate}
        \item We assume that $\mu < 1$ and that the fundamental strip of $K$ is $[0, \infty)$.  
    ...
    \end{enumerate}
\end{hypothesis}
We note that the first assumption has a biological interpretation: we want the mutation probability to be less than the natural growth of the population. The fact that the  fundamental strip of $K$ is $[0, \infty)$ means that $k_0$ decrease quickly in $0$ and in $\infty$, \ie 
\begin{equation*}
    \begin{cases}
        k(z) = o\left( \frac{1}{z^s}\right) & \text{for every } s \in (1, \infty) \text{ when } z \to \infty \eqfinv\\
        k(z) = o\left( \frac{1}{z^s}\right) & \text{for every } s \in (0, 1) \text{ when } z \to 0 \eqfinp
    \end{cases}
\end{equation*}
The second assumption means that $\mu$ is small enough so that  such an interval exists. For instance if $\mu<\frac{1}{2}$ we have $K(1)=1<\frac{1}{\mu}-1$. The fact that $\mu$ is small means that mutations are rare events. 
...
Since $\tilde \varphi$ is analytic on $\RR_+$, it follows that for all $x \in \RR_+$ 
...
Let us denote
    ...
    \end{align}
for a given $\lambda_0 \in D$ and \blue{$z \in \CC \setminus \RR_-$}. We notice that for $\lambda \in (0,\infty)$ we have $\varphi(\lambda)=1-\mu(1-K(s_0-\f{i}{2\pi} \log(\lambda))$.\\
...
\begin{itemize}
    ...
    \item  (Step 3) The quantity $\ln\left(\frac{\varphi(\lambda)}{z} \right)$ is bounded. 
\end{itemize}
...
\begin{remark}
    ...
    The function $V$ has no zero on the vertical strip $<s_0, s_0+1>$ \ndr{because $m$ is defined with an exponential}. Let $s_{pole} \in \CC$ be a pole of $\Phi$. Then $V$ has a zero for all $s \in \{s_{pole} - \NN^* \}$.
...
\subsection{Proof of the theorem \ref{t:existence_lineage}}
