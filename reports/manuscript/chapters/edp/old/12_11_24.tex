...
\begin{itemize}
    \item Ajout d'une majoration du terme $B$ dans la section 3.2.
    \item Déplacement de la section 4 en appendix C 
    \item Apport des corrections sur la section 2. 
    \item Réécriture de la section~\ref{s:bio_inter_courbe}
\end{itemize}
...
\begin{lemma}\label{lm:interval_mellin}
    ...
    $$ 
    is an interval of $\RR$ and the application $s \mapsto \Mcal_f(s)$ is analytic on the strip $I(f) + i\RR$. 
...
\label{s:lineage}
    ...
    An important tool to study Equation~\eqref{eq:main} is the Mellin transform that we defined in \eqref{e:mellin_def_function}. Indeed, by applying the Mellin transform to Equation~\eqref{eq:main}, we have
	\begin{equation*}
        \begin{cases}
...
\end{align*}
    ...
    %However, there is no guarantee that $K$ admits a local minimum \cite{doumic2016time}. The following lemma shows that if $K$ is strictly monotonic then all mutations are beneficial or all mutations are deleterious. Since we want to study kernels that can have both beneficial and deleterious effects, we assume that $K$ has a unique minimum reach in $s_* \in \RR$.
        ...
        \begin{proposition}
            \label{p:locationmoment} Suppose that $K$ is minimum in $s_* \in (0, \infty)$, then for all $R > 0$
            \begin{enumerate}
                \item 
                \begin{enumerate}
        ...
        \end{align*} 
        This ensures that for any compact set $ C \subset (0, \infty)$, the set  $$\{ x \in C \,;\, u(t,x) > 0\}$$ has measure $0$ when $t \in \R_+$ is enough large. For any real $A > 0$, this means that the whole population has a fitness greater than $A$ after a sufficiently long time.
...
Our aim is to study the asymptotic behaviour of $w$ as $t$ becomes larger and larger. For that, we apply the method of steepest descent which leads us to consider the curve $(t,x) \mapsto s_+(t,x)$ where $\partial_s \phi(s_+, \cdot,\cdot)= 0$. 
...
We observe that the sign of $F(r)$ implies the exponential growth or decay of $u(t, x)$ on the curve $\Ccal_{r}$. \\
    ...
    \ndr{Malheureusement... compliqué d'avoir une information précise sur ce maximum... ne serait-ce que pour montrer qu'il est unique ? }\\
...
\end{align*}
To obtain a bound on $B$, we start by obtain a bound on $B_1$. \\
\ndr{We can't use the properties of section $B$. It means we have to add hypothesis on the kernel.}
