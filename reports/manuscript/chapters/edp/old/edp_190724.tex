...
\begin{remark}
    ...
    The function $V$ has no zero on the vertical strip $<s_0, s_0+1>$ \ndr{because $m$ is defined with an exponentiel}. Let $s_p \in \CC$ be a pole of $\Phi$. Then $V$ has a zero for all $s \in \{s_p - \NN^* \}$.
    Let $s_p \in \CC$ be a zero of $\Phi$. Then $V$ has a zero in $s_p + \NN$. 
\end{remark}
...
The main contribution of $\Mcal^{-1}[L(z,\cdot)](x)$ as $x \to \infty$ is due to the closest pole of $L(z,\cdot)$ to the line $b+i\RR$ and right to this line \ndr{to develop}. The pole of $L(z,\cdot)$ are given by the zeros of $V$. 

Let $s_p \in \CC$ be a such pole of $L$ \ndr{existence ?}. By deforming the contour integration to a line $d+i\RR$ with $d > \Re(s_p)$, The residue theorem ensures that
\begin{subequations}
\begin{align}
    \Mcal^{-1}[L(z,\cdot)](x) 
...
$$ \res\left( \frac{1}{V(s)}; s = s_p\right) = \frac{1-\mu}{V(s_p-1)} \frac{1}{} \eqfinp$$
...
We want to prove something like that \ndr{we would like to force $\alpha = \log\big(z/(1-\mu)\big)$}
\begin{align*}
    \Lcal^{-1}\left[ \frac{e^{ \alpha(z)(\sigma-s_p)}}{z}  \right]
    &= 
    \int_{c-i\infty}^{c+i\infty} e^{ \alpha(z)(\sigma-s_p)}e^{tz}\frac{\diff z}{z}\\
    &=  \Gamma(\sigma-s_p)((1-\mu)t)^{-(\sigma-s_p)}(e^{2i\pi(\sigma-s_p)}-1)
\end{align*}
