...
\ln(z) := \ln(|z|) +{\MD i\theta \eqsepv -\pi \leq \theta < \pi } \eqfinp
\end{equation}
Let us denote
...
\begin{align*}
    ...
    C(z,\zeta) = C(z)
\end{align*}
\ndr{Pas de problème quand $\zeta\to 0$ car la fonction est analytique... donc bornée sur un disque centré en $0$ par le principe du maximum.}
Then 
\begin{align}
...
\begin{equation*}
    m(z,s) = V(s)
    \exp \left[
    - \int\limits_{s_0-i\infty}^{s_0+i\infty} \left[\log \left( \frac{1-\mu}{z}\right) + 2i\pi n\right]
    \cdot \left(
        \frac{1}{1-e^{2i\pi(s-\sigma)}} - \frac{1}{1-e^{2i\pi(\sigma_0-\sigma)}}
     \right) \diff \sigma
    \right]
...
where $$E(z,y,s) := \exp \left[
    - \int_{s_0-i\infty}^{s_0+i\infty} \left[\log \left( \frac{1-\mu}{z}\right) + 2i\pi n\right]
    \cdot \left(
...
\begin{equation*}
    ...
    \exp
        \left[
     \left(\log \left( \frac{1-\mu}{z}\right) + 2i\pi n\right)
    \cdot (y-s)\right]
\end{equation*}
