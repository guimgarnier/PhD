...
Define $$ s_+(t,x) := (K')^{-1} \Big( \frac{\log x}{\mu t} \Big) \eqfinp$$
...
\subsection{Reformulating the equation}\mbox{}
...
The first step is to transform the Equation~\eqref{eq:wienerhopfeq} on a strip into an equation on $\CC \setminus \RR_+$. For this, we introduce the variables:
    \begin{align}
        \zeta &= T(s) = e^{2i\pi(s-s_0)}, \label{def:zeta}\\
        g(z,\zeta) &= L(z,s), \label{def:g} \\
...
Now we want to return to the solution of Equation~\eqref{eq:wienerhopfeq}. For this purpose, we revert the change of variables \eqref{def:zeta},\eqref{def:g} and \eqref{def:Phi}.
\begin{align}
    L(z,s) = g(z, T(s)) 
    \eqfinp
\end{align}
